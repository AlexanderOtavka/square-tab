...
class Settings {
  ...
  static get _defaults() {
    return {
      ...
      [this.keys.BOOKMARKS_DRAWER_POSITION]: this.enums.BookmarkDrawerPositions
        .RIGHT,
      [this.keys.BOOKMARKS_DRAWER_SMALL]: true,
      [this.keys.SHOW_PHOTO_SOURCE]: true,
      [this.keys.BOXED_INFO]: true,
      [this.keys.TWENTY_FOUR_HOUR_TIME]: false,
  ...
  static get _overrides() {
    return {
      ...
      [this.keys.BOOKMARKS_DRAWER_MODE]: value => {
        if (
          value === this.enums.BookmarkDrawerModes.TOGGLE ||
  ...
  static main() {
    this._storageKeysArray = Object.keys(this.keys).map(
      keyName => this.keys[keyName]
    )
