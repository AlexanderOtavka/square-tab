...
class BookmarksNavigator {
  ...
  static main() {
    ...
    chrome.bookmarks.onMoved.addListener((id, moveInfo) => {
      if (
        moveInfo.parentId === this.currentFolder &&
        ...
        if (element.node.id === id) {
          ...
          const beforeElement = this.$drawerItems.childNodes[moveInfo.index]
      ...
      if (!node.url) {
        ...
        chrome.bookmarks.getChildren(id, children => {
          ...
          while (children.length < elements.length) {
            this.$drawerItems.removeChild(this.$drawerItems.lastChild)
          }
          ...
          const $hoveredBookmark = this.$drawerItems.querySelector(
            "x-bookmark:hover"
          )
          if ($hoveredBookmark) {
