...
export function onBookmarkDragStart(ev) {
...
export function onBookmarkDragOver(ev) {
  const index = Array.prototype.indexOf.call($drawerItems.childNodes, ev.target)
  handleDragOver(ev.target, index, ev.detail.y)
}

export function onBookmarkDrop(ev) {
  const index = Array.prototype.indexOf.call($drawerItems.childNodes, ev.target)
  handleDrop(ev.target, index, ev.detail)
}

export function onItemsDragOver(ev) {
  ev.preventDefault()
...
export function onItemsDrop(ev) {
  ev.preventDefault()
...
export function onUpButtonDragOver(ev) {
  ev.preventDefault()
...
export function onUpButtonDragLeave(ev) {
  if (ev.target === $upButton) {
    $upButton.classList.remove("expand")
  }
}

export function onUpButtonDrop(ev) {
  ev.preventDefault()
  $upButton.classList.remove("expand")
...
export function openCtxMenu(x, y, nodeId) {
...
function handleDrop(target, index, detail) {
  ...
  const draggedElement = currentDraggedBookmark
  const draggedIsEditable =
    !draggedElement || BookmarksNavigator.nodeIsEditable(draggedElement.node.id)
