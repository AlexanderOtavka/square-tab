 ...
 */
export function maybeShowTooltipForBookmark(bookmark) {
...
export function onBookmarkMouseOver(ev) {
  maybeShowTooltipForBookmark(ev.target)
}

export function onHeaderMouseOver() {
  if (Settings.get(Settings.keys.BOOKMARKS_DRAWER_SMALL)) {
    $drawerTooltip.show($header, $title.title)
  }
}

export function getNodeTitle({ id, url, title }) {
  if (title || url) {
    return title || url
  } else if (id === ROOT_ID) {
    return "Bookmarks"
...
export function nodeIsEditable(nodeId) {
  return (
    nodeId &&
    nodeId !== ROOT_ID &&
...
export function openBookmark(id) {
  if (id !== getCurrentFolder()) {
    stack.push(id)
  }
  ...
  chrome.bookmarks.get(id, ([node]) => {
    if (!node.url) {
      ...
      chrome.bookmarks.getChildren(id, children => {
        ...
        const $hoveredBookmark = $drawerItems.querySelector("x-bookmark:hover")
        if ($hoveredBookmark) {
...
export function updateSize(small) {
